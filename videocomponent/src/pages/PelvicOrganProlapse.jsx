
import './PelvicOrganProlapse.css';
import PelvicOrganProlapse from './assets/pelvic organ prolapse.png';
import PelvicOrganProlapse1 from './assets/pelvic organ prolapse(1).png';
import SubtopicSidebar from './SubtopicSidebar';

const PelvicProlapseArticle = () => {
  return (
    <div className="main">
      <article className='urinary-incontinence-article'>
      <header className="article-header">
        <h1>Pelvic Organ Prolapse</h1>
        <p className="intro-text">
        Pelvic organ prolapse, also known as urogenital prolapse, refers to the <strong>downward displacement of pelvic organs, leading to the protrusion of the vagina, uterus, or both.</strong> This condition is unique to women and may involve the anterior or posterior vaginal wall, the uterus, or the vaginal apex, often occurring in various combinations. Pelvic organ prolapse differs from rectal prolapse, a condition in which the rectum protrudes through the anus and can affect both men and women.
        </p>
        <p>This condition is not life-threatening but disturbs the quality of life.</p>
        
      </header>

      <section className="types-section">
      <h3>Types:</h3><div>
        <ol>
            <li><strong>Uterine Prolapse-</strong>The uterus drops down into the vagina. In severe uterine prolapse, the uterus may protrude from the vagina.</li>
            <li><strong>Anterior Vaginal Wall Prolapse-</strong>The bladder drops down and creates a bulge into the front wall of the vagina. In severe cases, the bulge can protrude outside the vagina. Another name for bladder prolapse is cystocele.</li>
            <li><strong>Posterior Vaginal Wall Prolapse-</strong>The rectum drops down and creates a bulge into the back wall of the vagina. In severe cases, the bulge can protrude outside the vagina. Another name for this condition is rectocele.</li>
            <li><strong>Uterovaginal Prolapse-</strong> In some cases, several organs can drop. Here the bladder, uterus, and rectum are shown shifting down.</li>
        </ol>
        <p>Although two-thirds of parous women exhibit anatomical evidence of pelvic organ prolapse (POP), most of them remain asymptomatic. Studies indicate that in the general population, <strong>40% of women aged between 45 and 85 years have objectively detectable POP upon examination, yet only 12% experience symptoms.</strong>
        </p>
        </div>
        
      </section>
      <section id='symptoms'>
      <img src={PelvicOrganProlapse} alt='Pelvic organ prolapse image' style={{width:'85%'}}/>
      </section>
      <section id='risk-factors'>
      <h2>RISK FACTORS</h2>
      <img src={PelvicOrganProlapse1} alt='Pelvic organ prolapse image' style={{width:'85%'}} />
      </section >
      <section className="screening-section" id='screening'>
        <h2>SCREENING</h2>
        <p>Routine screening during annual well-woman visits is essential for detecting pelvic organ prolapse (POP). Many patients may not openly discuss symptoms due to embarrassment, so focusing on urinary symptoms and the sensation of a vaginal bulge is crucial. This proactive approach helps identify those who might be reluctant to bring up their symptoms.</p>

        <ul>
          <li>A comprehensive medical, surgical, obstetric, and gynecologic history should be collected.</li>
          <li>The nature and impact of vaginal bulge symptoms should be documented.</li>
          <li>Limitations on physical activities, sexual function, and symptom progression should be assessed.</li>
          <li>Lower urinary tract function should be evaluated, including urine loss, type of incontinence (stress or urgency), and bladder emptying.</li>
          <li>Bowel function should be reviewed for straining, laxative use, fecal incontinence, and incomplete rectal emptying.</li>
          <li>Symptoms such as dyspareunia (painful intercourse), coital incontinence, and sexual dysfunction related to prolapse should be inquired about.</li>
        </ul>
      </section>

      <section className="diagnosis-section" id='diagnosis'>
        <h2>DIAGNOSIS</h2>
        <div className="diagnosis-content">
          <h3>Physical Examination</h3>
          <p>A comprehensive physical examination is crucial for diagnosing and classifying pelvic organ prolapse (POP). Initially, an abdominal and pelvic examination should be conducted to exclude pelvic masses. The external genitalia and vaginal epithelium need to be inspected for atrophy, irritation, or ulceration. A detailed assessment of the prolapse is performed using a split speculum while the patient performs a Valsalva maneuver or coughs. This examination helps to reveal the perineal body and vaginal opening, and the anterior and posterior vaginal walls are inspected for cystocele and rectocele using a 1-blade Sims speculum. Additionally, pelvic floor muscle tone should be assessed, noting the strength of contractions. The extent of prolapse is quantified using either the Baden-Walker grading system or the Pelvic Organ Prolapse Quantification (POP-Q) system to provide a structured clinical assessment.</p>
          <ul>
            <li><strong>Pelvic Organ Prolapse Quantification(POP -Q)Examination:</strong></li>
            <li>
                <ul>
                    <li>Recommended to objectively evaluate and document the extent of prolapse.</li>
                    <li>The POP-Q system measures prolapse in the anterior, apical, and posterior compartments.</li>
                    <li>Provides consistency in reporting and aids communication among care providers.</li>
                </ul>
            </li>
            <li><strong>Urodynamic Testing:</strong> A urodynamic evaluation is recommended for patients with significant voiding symptoms to assess bladder and sphincteric function. This test helps identify detrusor dysfunction and high post-void residual, which are common in severe prolapse. Reducing the prolapse during the evaluation can unmask stress urinary incontinence and improve the assessment of sphincteric function.</li>
            <li><strong>Imaging:</strong>In cases of severe procidentia, imaging such as a computed tomography (CT) urogram is indicated. This helps evaluate potential distortion of the pelvic anatomy, particularly the ureters, which might be affected by the descent of the bladder, leading to obstruction and hydronephrosis.</li>
          </ul>
        </div>
      </section>

      <section className="treatment-section" id='treatment'>
        <h2>TREATMENT</h2>
        <div className="treatment-content">
            <p>Several treatment options are available for the management of pelvic organ prolapse (POP). These include both conservative and surgical approaches, with pelvic floor muscle training (PFMT) and vaginal pessaries being two primary non-surgical options.</p>
          <h3>Pelvic Floor Muscle Training (PFMT)</h3>
          <p>PFMT is a structured and individualized program of exercises aimed at strengthening the pelvic floor muscles to address symptoms of both stress urinary incontinence (SUI) and POP. The primary goal of PFMT is to enhance the muscle strength, endurance, and power of the pelvic floor, which can be done through repeated contractions. PFMT can also improve relaxation and conscious muscle pre-contraction before increases in abdominal pressure, such as during coughing.</p>
          <ul>
            <li><strong>Postpartum POP and PFMT:</strong><p>While evidence of PFMTs effectiveness on postpartum POP remains limited, regimented PFMT has shown some improvement in POP symptoms within 12 months postpartum, though not necessarily in the anatomical stage.</p>
          <p>The National Institute of Health and Care Excellence recommends a minimum of a 3-month trial of supervised PFMT as the first-line treatment for SUI, which may also apply to POP management. The effectiveness of PFMT is heightened when treatment protocols are individualized and supervised, with higher patient-reported cure rates and greater reduction of symptoms.</p>
</li>
          <li><h3>Vaginal Pessaries</h3>
          <p>A vaginal pessary is an intra-vaginal device used to support vaginal wall prolapse or manage urinary incontinence. Pessaries are commonly used in women seeking conservative treatment for POP or as a temporary solution while awaiting surgical correction.</p>
          <p>It is estimated that 71–90% of women can be successfully fitted with a pessary for POP, and 70–90% of those who are successfully fitted report symptomatic relief. Pessaries work by supporting the pelvic organs, alleviating the discomfort and symptoms associated with POP.</p>
          <p>These are additional non-surgical techniques for preventing pelvic organ prolapse (POP):</p>
          <ul>
            <li>stopping smoking</li>
            <li>decreasing pushing/straining for bowel movements</li>
            <li>decreasing weight</li>
          </ul></li>
          <li>
          <h3>Midurethral Sling(MUS)</h3>
          <p>The midurethral sling (MUS) is a widely used, minimally invasive surgical procedure for treating stress urinary incontinence (SUI) that can accompany POP. The procedure involves the placement of a synthetic mesh to provide support to the urethra. MUS has symptom cure rates ranging from 75% to 94%, with objective cure rates from 57% to 92%.</p></li>
          <li>
            <h3>Burch Urethropexy</h3>
            <p>The Burch retropubic urethropexy is an older surgical procedure involving the fixation of the urethra to Cooper’s ligament via an abdominal approach. It was once the gold standard before MUS gained popularity.
            Studies show it remains effective in the long term, especially when compared to traditional autologous pubovaginal slings (PVS).</p>
          </li>
          <li>
            <h3>Autologous Pubovaginal Sling(PVS)</h3>
            <p>The autologous pubovaginal sling (PVS) is a more invasive option for SUI treatment, where a sling is created using the patient’s own tissue. It is typically reserved for patients with recurrent or severe SUI, particularly those with fixed urethra or complications from previous surgeries.</p>
            <p>The key advantage of PVS is the absence of synthetic materials, eliminating the risk of mesh-related complications. However, it comes with a higher surgical burden, including longer operative times, greater blood loss, and postoperative complications like voiding dysfunction.</p>

          </li>
          <li>
            <h3>Urethral Bulking</h3>
            <p>For women who cannot or do not wish to undergo surgery, urethral bulking involves injecting agents such as silicone particles to provide support for the urethra.</p>
          </li>
          <li>
            <h3>Artificial Urethral Sphincter(AUS)</h3>
            <p>The AUS may be considered for women with severe intrinsic sphincter deficiency (ISD). However, due to the high risk of complications like vaginal erosion, it is not commonly recommended as a first-line treatment for SUI in female patients.</p>
          </li>
          <li>
            <h3>Native Tissue POP Repairs</h3>
            <p>Native tissue repairs use the patients own tissue and are often recommended for women who wish to remain sexually active. Procedures like sacrospinous ligament suspension (SSLS) and uterosacral ligament plication (USLP) are common. </p>
          </li>
          <li>
            <h3>Mesh-Based POP Repairs</h3>
            <p>Mesh-based repairs, particularly transvaginal mesh (TVM), have declined due to complications, with re-operation rates 2.4 times higher than native tissue repairs. SCP, performed abdominally, laparoscopically, or robotically, remains the gold standard for POP, offering a high success rate (78–100% at 3 years) but a 10.5% risk of mesh erosion by 7 years. Uterine-preserving mesh-based surgeries are gaining popularity due to reduced risks, but long-term data are limited. ACOG recommends these repairs only when other treatment options are not appropriate.</p>
          </li>
          </ul>
        </div>
      </section>

      <section className="resources-section" id='case-studies'>
        <h2>CASE STUDIES</h2>
        <div className="services-grid">
          <div className="service-card">
            <ul>
              <li>
                <h3>High Income Country</h3>
                <p><strong>Trends in pelvic organ prolapse management in Latin America</strong></p>
                <p><strong>Study:</strong>The survey, which was assembled in Google Forms in both Spanish and Portuguese, was distributed via email to members of the CAU and gynecologic associations under the Federación Latinoamericana de Sociedades de Obstetricia y Ginecología (FLASOG). These associations comprise more than 30,000 members. Participants needed to have valid credentials and be active members of the associations in their respective countries to meet the inclusion criteria, while retired professionals were excluded. The survey was conducted between April and September 2016, consisting of 37 items aimed at characterizing the population, technical aspects of surgeries and procedures, theoretical knowledge, practice patterns, opinions, and perceptions.</p>
                <p>A total of 673 responses were collected, with 529 (78.6%) participants meeting the inclusion criterion of performing at least one pelvic organ prolapse (POP) procedure per month. The majority of respondents were urologists (61%), followed by gynecologists (29.5%) and urogynecologists (9.5%).</p>
                <p><strong>Outcome:</strong> The majority of respondents (59.5%) used the POP-Q classification system, with urogynecologists showing the highest adoption rate (90%) compared to urologists (54.2%) and gynecologists (60.9%). In terms of surgical approach for apical POP, 55.6% of participants preferred the vaginal route, while 26.4% preferred abdominal surgery. Most respondents (37.6%) performed prophylactic management for stress urinary incontinence (SUI) during a POP procedure.The survey revealed that 75.2% of respondents reported a decline in the use of mesh due to FDA warnings, while 41.9% believed that these warnings had negatively impacted the use of incontinence tapes as well. Despite these concerns, 75.8% of participants indicated that they would still consider mesh repairs in select cases. The most commonly performed vaginal procedures among non-mesh users included sacrospinous fixation (39.8%), anterior or posterior colporrhaphy (32.9%), and uterosacral fixation (15.4%).A key observation was the disparity in training across different specialties. While 93.6% of gynecologists reported receiving POP training during residency, only 66.6% of urologists and 76% of urogynecologists received similar training. Furthermore, only 35.9% of gynecologists reported using mesh for POP repairs, compared to 65% of urologists and 72% of urogynecologists.</p>
                <p><strong>Discussion:</strong>Despite FDA warnings, many Latin American practitioners still favor mesh procedures for specific cases. Unlike North America and Europe, where mesh use has dropped, preference for vaginal surgeries remains consistent globally. Urologists, with more mesh training, use mesh three times more often than gynecologists, highlighting the need for standardized training across specialties.</p>
              </li>
            <li>
                <h3>Clinical Profile of Uterine Prolapse Cases in South India</h3>
                <p><strong>Study:</strong>The study intervention involved a comprehensive review of medical case records for confirmed uterine prolapse (UP) cases admitted between 2009 and 2014 at Government Lady Goshen Hospital and Kasturba Medical College Hospital, Mangalore. This included collecting and analyzing data on socio-demographic details, obstetric history, presenting complaints, degree of UP, associated pelvic prolapse, and management strategies using a validated proforma and SPSS software. The primary management approaches documented were surgical interventions such as vaginal hysterectomy (in 252 cases) and Manchester repair (in 3 cases), as well as conservative methods like ring pessary use (in 11 cases) and Kegel exercises (recommended to 5 cases).</p>
                <p><strong>Outcomes:</strong>The mean age of patients was 56.4 ± 12.3 years, with the majority being manual laborers (78.6%), from poor socio-economic backgrounds (76%), and rural areas (62.8%). The mean age at presentation was 52.8 ± 13.2 years, with a significant positive correlation with age at first and last pregnancy. Obstetric risk factors included delivery by untrained personnel (25.3%) and lack of episiotomy (78%). The majority had a history of heavy postnatal work (29.8%) and some had a history of unsafe abortion (7%). Chronic cough (27.6%) and constipation (8.1%) were also noted. Third-degree UP was most common (76.8%), with the most frequent associated condition being cystocele (74.6%). Hypertension (34.9%) was the most common co-morbidity. Vaginal hysterectomy was the most common surgical procedure performed, with only 3.1% receiving conservative management with ring pessaries.</p>
                <p><strong>Discussion:</strong>The study indicates that UP predominantly affects older women, particularly those of lower socio-economic status and engaged in manual labor, aligning with findings from similar studies. The observed age at presentation and the association with obstetric factors such as untrained delivery personnel and lack of episiotomy reflect established risk factors for UP. The high incidence of third-degree prolapse and common co-morbidities like hypertension and diabetes further illustrate the complex interplay of socio-economic and health factors in UP(uterine prolapse) cases. While surgical management, primarily vaginal hysterectomy, is prevalent, conservative methods such as ring pessaries and Kegel exercises are underutilized despite their potential benefits. This highlights the need for improved education on conservative management options and emphasizes the importance of addressing socio-economic and health disparities in preventive measures and treatment strategies for UP.</p>
            </li>
            <li>
                <h3>Low-Income Country</h3>
                <p><strong>Study:</strong>The intervention in this study involved the provision of free surgical treatment for severe stages of pelvic organ prolapse to 24 women, facilitated through a newly initiated campaign at the University of Gondar Hospital. The intervention included a comprehensive approach involving pre- and post-operative care, as well as follow-up assessments to evaluate the outcomes of the surgical treatment. Additionally, the study explored alternative healing practices, such as the use of tsebel (holy water) and local healers, by incorporating visits to holy water sites and interactions with traditional practitioners. This multi-faceted approach aimed to capture a holistic view of health care seeking behaviors among women with pelvic organ prolapse in Ethiopia.</p>
                <p><strong>Outcomes:</strong>The study focused on 24 Ethiopian women with pelvic organ prolapse (POP) revealed that the average age was 46 years, with most being married and having experienced early marriages and deliveries. The majority had stage III prolapse, with many suffering for over a decade. Most women initially sought help from local health centers but faced barriers like financial constraints and negative perceptions of surgery. Many turned to local healers and holy water before considering hospital treatment.
                A new community mobilization program significantly increased awareness of surgical options, leading to a shift in perceptions and greater willingness to seek hospital care. Structural barriers such as poverty, long distances to health facilities, and gender norms were substantial obstacles. </p>
                <p><strong>Discussion:</strong>The study highlights the influence of cultural beliefs, financial limitations, and structural barriers on women’s healthcare decisions in rural Ethiopia. Traditional remedies and spiritual beliefs were primary factors in healthcare-seeking behavior. Financial and logistical barriers prevented many from accessing surgical treatment, while gender norms and early marriages limited womens ability to seek timely care. The community mobilization program proved effective in increasing awareness and changing perceptions about prolapse treatment, showing promise for integrating traditional and modern care approaches.</p>
            </li>
            </ul>
          </div>
          
        </div>
      </section>
      <section>
      <div className="service-card" id='specialists'>
            <h3>SPECIALISTS</h3>
            <p>Urogynecologists, Obsteticians and Gynecologists, Colorectal Surgeons, Physical Therapists(specialized in pelvic floor therapy),Urologists.</p>
          </div>
          </section>
          <SubtopicSidebar />
          </article>
    </div>
  );
};

export default PelvicProlapseArticle;